...
| `tools.spatial_oracle.aircraft_approach_range` | 235 original Object Distance_To queries and Aircraft range branches: strict raw-lepton boundary, sqrt ties, elite fallback, cell targets and all 22 foundations with Bib both ways | `combat::object_distance_to`; `aircraft/approach_range_tests.rs` drives the production mission/move dispatcher and save/restore. Excludes the native branch admission, other mission states and firing |
...
| `tools.spatial_oracle.tag_lifecycle` | 39 retained original histories: shared versus fresh Tags, reversed linked execution, repeat modes/refcounts, cell detach/enqueue, completion bits, timer overflow/reset/RNG and force/enable/disable | Evidence for the pending live-instance migration. Supplies allocation storage and initialized registries; no gameplay calls replaced. Excludes final destruction, object/Team lifecycle and production scenario dispatch; no Rust parity claim |
