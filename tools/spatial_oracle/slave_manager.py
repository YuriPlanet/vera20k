...
from tools.spatial_oracle.refinery_dock import HOUSE, RULES, cell, cell_xy, place_building
...
MISSION = {'guard': 5, 'move': 2, 'harvest': 10, 'construction': 18, 'selling': 19, 'none': -1}
...
def make_fixture(case):
    ...
    u.mem_write(MANAGER + 0x24, dwords(YAREFN, STYPE, len(nodes), case.get('regen', 500), case.get('reload', 25)))
    u.mem_write(MANAGER + 0x38, dwords(0x7F322C, ITEMS, 16) + bytes([1, 0, 0, 0]) + dwords(len(nodes), 10))
    u.mem_write(MANAGER + 0x50, dwords(frame, 0, 10, case.get('manager_state', 5),
                                      case.get('manager_frame', 0)))
    u.mem_write(YAREFN + 0x2D8, dwords(MANAGER))
    for index, node in enumerate(nodes):
        ...
        if slave:
            make_slave(u, call, slave, node)
    make_slave(u, call, slave_address(SPARE), dict(limbo=True))
    return u, call, read32, events


def make_slave(u, call, slave, node):
    loco = slave + 0x1000
    ...
    u.mem_write(slave + 0x2DC, dwords(YAREFN))
...
def run(case, entry, this, args=()):
    ...
    run_checked(u, entry, RET_MAGIC, count=5_000_000, required_addresses=[entry])
...
def state(u, read32, case):
    ...
    for index in sorted(set(present + [SPARE])):
        ...
        slaves[str(index)] = dict(
            ...
            health=[signed(slave + 0x6C), signed(slave + 0x70)], owner=read32(slave + 0x2DC) == YAREFN,
            limbo=u.mem_read(slave + 0x81, 1)[0], coord=list(struct.unpack('<3i', u.mem_read(slave + 0x9C, 12))))
    base = field.field_state(u, read32, case)
    return dict(nodes=nodes, slaves=slaves, manager_state=signed(MANAGER + 0x5C),
    ...
    finish_vectors(
        ...
        provenance=lambda: provenance(
            scope='SlaveManagerClass AI_Update 0x6AF6C0, manager state machine 0x6AFD60 (Building owner, '
                  'states 0/5/6), DeploySlaves 0x6B04C0, InfantryClass::Mission_Harvest 0x522E70 and the '
                  'slave deposit 0x522D50',
