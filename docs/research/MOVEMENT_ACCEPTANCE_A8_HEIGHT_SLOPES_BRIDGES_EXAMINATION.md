...
survives where the destination land row is below 100 percent - CORRECTED 2026-09-16 from "about 0.83", which is wrong: a 0.9 row gives native min(0.9 x 1.2, 1.0) = 1.0 against VERA's 0.9, and those differ - because
...
|---|---|
| D1's frequency (how many retail ramp exits sit on a sub-100% land row) | Extend `retail_high_bridge_inventory` / a new ramp inventory over `Hills.mmx`, `BayOPigs.mmx`, `Deadman.mmx` to report each ramp-adjacent cell's land row per SpeedType. Cheap; needs only the harness. |
