...
|---|---|---|
| U1 **RESOLVED 2026-09-16** | The RTTI ids: `vtable+0x2C == 0xF` is **InfantryClass**, `== 6` is **BuildingClass**. | Settled by the first method this row proposed, the RTTI/COL substrate, with no breakpoint. InfantryClass: TypeDescriptor `.?AVInfantryClass@@` at `0x00825508`, COL `0x008033B8`, vtable `0x007EB058`, slot `+0x2C` = `0x007EB084` holding `0x00523340` = `MOV EAX,0xF` / `RET`. BuildingClass: `.?AVBuildingClass@@` at `0x00818D60`, COL `0x007FC360`, vtable `0x007E3EBC`, slot `+0x2C` = `0x007E3EE8` holding `0x00459EC0` = `MOV EAX,6` / `RET`. Both COLs carry signature 0 and offset 0, so these are the primary-base vtables a `vtable+0x2C` call on a Foot pointer dispatches through, and each body has exactly one reference - a DATA reference from its own vtable slot, not a call site. The row's warning was right: a brute `MOV EAX,0xF` search returns three hits, two of them mid-function, and would not have settled it. |
