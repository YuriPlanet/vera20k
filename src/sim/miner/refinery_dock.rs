//! War Miner refinery dock as the native Enter and Unload missions.
//!
...
//!
//! Scope: the War Miner. The Chrono Miner keeps the legacy phase machine
//! (`miner_dock_sequence`) until its teleport return chain: its pad approach
//! is the Unit setter's Teleporter arm (`0x007423CD`), not a drive.
//!
...
//! - Mission_Enter's Teleporter re-assign (`0x004D93E8`) and IPiggyback release
//!   (`0x004D931E`) are Chrono Miner arms.
...
/// Whether `id` runs the native dock missions: a War Miner.
pub(crate) fn native_dock_miner(sim: &Simulation, id: u64) -> bool {
    sim.substrate
        .entities
        .get(id)
        .and_then(|entity| entity.miner.as_ref())
        .is_some_and(|miner| miner.kind == MinerKind::War)
}

/// `FootClass::Mission_Enter @ 0x004D9290` for a War Miner. Returns the
/// dispatch delay.
...
/// setter takes `NavQueue[0]` with the flag 0 (the queue is kept) and the
/// first entry is then deleted. Only Cell waypoints are represented.
fn pop_nav_queue(sim: &mut Simulation, rules: &RuleSet, id: u64) {
    let Some(entity) = sim.substrate.entities.get(id) else {
        return;
    };
