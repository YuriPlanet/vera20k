...
/// `UnitClass::Can_Enter_Cell @ 0x0073F0A0` asked from a standing object,
/// as `FootClass::Is_Cell_Harvestable` asks it (`(cell, -1, -1, 0, 1)` at
/// `0x004DCFAF`): the ground land row for the mover's SpeedType, then the
/// ground object list with the building entry exceptions a crossing applies
/// (a building in radio contact, the repair/bunker rows, a bib, an open
/// gate). Returns the YR code; 0 is MOVE_OK.
///
/// RESIDUAL: the `-1` height selects the deck list of a bridge-stamped cell
/// (`0x0073F0B7`); this standing query reads the ground list. Only ore
/// beneath a bridge deck could tell them apart.
pub(crate) fn unit_can_enter_cell_standing(
    sim: &crate::sim::world::Simulation,
    mover_id: u64,
    cell: (u16, u16),
    rules: &crate::rules::ruleset::RuleSet,
) -> u8 {
    let Some(mover) = sim.substrate.entities.get(mover_id) else {
        return 7;
    };
    let locomotor = mover.locomotor.as_ref();
    ...
    let locomotor_kind = locomotor.map_or(LocomotorKind::Drive, |state| state.effective_kind());
    let grid = sim.path_grid.as_deref();
    let cost_grid = sim.terrain_costs.get(&speed_type);
    let terrain_passable = grid.is_none_or(|grid| {
        crate::sim::pathfinding::is_cell_passable_for_mover_with_speed(
            ...
            cost_grid,
            false,
            cell_entry::TerrainEntryMode::RuntimeTransition,
        )
    });
    if !terrain_passable {
        return 7;
    ...
    match cell_entry::check_terrain_with_layers(
        cell,
        layers,
        mover.category,
        grid,
        ...
        cell_entry::TerrainCheckResult::Clear => 0,
        cell_entry::TerrainCheckResult::Impassable => 7,
        cell_entry::TerrainCheckResult::NeedsBlockerCheck => {
            let facts = MoverBuildingEntryFacts::new(mover, Some(rules));
            let skips = LiveBuildingEntrySkips {
                mover: facts.as_ref(),
                others: crate::sim::entity_store::OtherEntities::whole(&sim.substrate.entities),
                rules: Some(rules),
                interner: &sim.interner,
                #[cfg(debug_assertions)]
                check: None,
            }
            .skips_at(cell, &sim.substrate.occupancy)
            .map(std::borrow::Cow::into_owned);
            cell_entry::classify_occupied_cell_with_layers_and_ignored(
                cell,
                layers,
                mover_id,
                bump_crush::CrushCapability::new(mover.regular_crusher, mover.omni_crusher),
                sim.interner.resolve(mover.owner()),
                locomotor_kind,
                false,
                skips.as_ref(),
                &sim.substrate.occupancy,
